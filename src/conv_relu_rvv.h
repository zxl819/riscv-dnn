...
static inline int conv(Tensor *dst, Tensor *src, Tensor *weight, Tensor *srcPad, Config *ss)
{
    ...
    for (int i = 0; i < hout; i++) {
      for (int j = 0; j < wout; j++) { 
        for (int k = 0; k < cout; k += vl_out) { // complete  vlmax one time
          vl_out = vsetvl_e16m4(cout - k);
        ...
        //   _sum = vfncvt_f_f_w_f16m4(_sum, vl_out) ;
        //   _sum = vfmax_vf_f16m4(_sum, base , vl_out);
          vse16_v_f16m4(pdst+offset_dst,  vfmax_vf_f16m4(vfncvt_f_f_w_f16m4(_sum, vl_out), base , vl_out), vl_out);
