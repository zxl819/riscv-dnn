...
// 回调接口：报告每个分块的起始索引与块大小
typedef void (*tile_iter_cb)(int i, int j, int kk,
                             int tile_m, int tile_n, int tile_k,
                             void* user);

// 按矩阵乘法三重循环格式进行分块遍历，仅调用 msettilem/msettilen/msettilek
static inline void tile_mnk_loop_cb(int m, int n, int k, tile_iter_cb cb, void* user) {
  ...
  for (int i = 0; i < m; i += tile_m) {
    ...
    for (int j = 0; j < n; j += tile_n) {
      ...
      for (int kk = 0; kk < k; kk += tile_k) {
        ...
        if (cb) cb(i, j, kk, tile_m, tile_n, tile_k, user);
      }
    }
  }
}
...
// 无回调便捷封装
static inline void tile_mnk_loop(int m, int n, int k) {
  tile_mnk_loop_cb(m, n, k, (tile_iter_cb)0, (void*)0);
}
