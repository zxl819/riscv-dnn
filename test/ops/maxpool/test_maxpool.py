...
def maxpool(hin, win, cin, kh, kw, sh=1, sw=1, pt=0, pb=0, pl=0, pr=0):
...
def test_maxpool(num, caseSize):
    ...
    if len(caseSize) >=11:
        ...
        pl = caseSize[9]
        pr = caseSize[10]
    maxpool(h, w, cin, kh, kw, stride_h, stride_w, pt, pb, pl , pr)
    if simulator == 'spike':
    ...
    for i in range(len(case_sizes)):
        ...
        for key,val in opt_level.items():
            os.system("clang++ -g --target=riscv64-unknown-elf  -march=rv64gcv -mattr=+zfh,+zvfh,+matrix \
                        ...
                        -O%d -o rvv-test test_maxpool.cpp  ../../src/mat.cpp" % val)
            
            tma = test_maxpool(key+'-'+str(i), case_sizes[i])
            if TEST_OPTION['perf']:    
                tma["Charis"] = key
                if val == 0:
